...
def test_read_subprocess_log_stream_warning_logs(mocker, subprocess_instance, mock_logger):
    """Test that read_subprocess_log_stream correctly handles WARNING level logs"""
    # Setup
    subprocess_instance.process_logger = mock_logger
    ...
    # Iteration sequence:
    # 1. First readline: returns warning message, poll returns None
    # 2. Second readline: returns empty, poll returns None
    # 3. Third readline: returns empty, poll returns 0 (terminates)

    mock_process.poll.side_effect = [None, None, 0]
    mock_process.stdout.readline.side_effect = [
        b'[WARNING] Warning message\n',  # First iteration: message logged
        b'',  # Second iteration: empty, still running
        b''  # Third iteration: empty, process terminated
    ]
