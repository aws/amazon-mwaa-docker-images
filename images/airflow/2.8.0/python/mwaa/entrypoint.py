...
# Our imports
from mwaa.config.airflow import get_airflow_config, get_db_connection_string


def abort(err_msg: str, exit_code=1) -> None:
    print(err_msg)
    sys.exit(exit_code)
...
def db_lock(lock_id, timeout=300 * 1000):
    def db_lock_specific(func):
        def wrapped(*args, **kwargs):
            func_name = func.__name__
            db_engine: Engine = create_engine(
                get_db_connection_string())  # type: ignore
            print(f'Obtaining lock for {func_name}...')
            with db_engine.connect() as conn:
                try:
                    conn.execute(text("SET LOCK_TIMEOUT to :timeout"),
                                 {"timeout": timeout})
                    conn.execute(text("SELECT pg_advisory_lock(:id)"),
                                 {"id": lock_id})
                    ...
                    try:
                        ...
                        abort(
                            f'Failed while executing {func_name}. ' +
                            f'Error: {e}.'
                        )
                except Exception as e:
                    abort(
                        f'Failed to obtain DB lock for {func_name}. ' +
                        f'Error: {e}.'
                    )
                finally:
        ...
        return wrapped
    return db_lock_specific


@db_lock(1234)
def airflow_db_init(environ):
    print("Calling 'airflow db migrate' to initialize the database.")
    response = subprocess.run(["airflow db migrate"],
                              shell=True, check=True, text=True, env=environ)
...
def export_env_variables(environ):
...
def main() -> None:
    ...
    try:
        ...
        print(sys.argv)
        exit('Invalid arguments. Please provide one argument with one of' +
