...
class ExitStatus(Enum):
    ...
    """
    SUCCESS=0
    INSUFFICIENT_ARGUEMENTS=1
    INVALID_AIRFLOW_COMPONENT=2
    AIRFLOW_COMPONENT_UNHEALTHY=3

def exit(exit_status: ExitStatus):
    """
...
def get_airflow_process_command(airflow_component: str):
    ...
    exit(ExitStatus.INVALID_AIRFLOW_COMPONENT)

if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("Usage: python3 healthcheck.py <airflow_component>")
        exit(ExitStatus.INSUFFICIENT_ARGUEMENTS)
    ...
    if is_process_running(airflow_cmd_substring):
        ...
        exit(ExitStatus.AIRFLOW_COMPONENT_UNHEALTHY)
