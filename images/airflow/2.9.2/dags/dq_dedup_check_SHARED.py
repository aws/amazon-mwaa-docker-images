...
def check_dq_complete():
    sql_dq_complete="""
        ...
        (
            ...
            FROM AI_DATAMART.DM.vw_dq_dedup_schema_results_today 
            WHERE 1=1
        ...
        dbt_task = BashOperator(
            ...
            bash_command="""
                ...
                dbt run \
                    ...
                    --select "dq_dedup_src_tgt_results_multiple_clients"
                """,
...
dag = DAG(
    ...
    dag_id="dq_dedup_check_SHARED",
    description="DQ DEDUP DAG for running dbt models for shared schemas that has pending DQ. Generates tasks based on schemas defined on schema_list",
    start_date=datetime(2024, 5, 1, tzinfo=local_tz),
    schedule_interval="1,16,31,46 1-15 * * *",
    concurrency=1,
    max_active_runs=1,
    catchup=False,
    tags=['BI', 'DQ'],
